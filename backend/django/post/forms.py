...
class PostForm(forms.ModelForm): # 모델에서 폼 불러오기
    ...
    tema = forms.CharField(label='테마',widget=forms.TextInput(attrs={'placeholder':'테마를 입력해주세요'}))
    cosmetic = forms.CharField(label='화장품',widget=forms.TextInput(attrs={'placeholder':'사용한 화장품을 입력해주세요'}))
